...
public class CartServiceImpl implements CartService {
    ...
    public void initialiseCartList() {
        ...
        BigDecimal bigDecimal1 = cartItemList.stream()
                .map(cartItem -> cartItem.getProduct().getPrice().multiply(BigDecimal.valueOf(cartItem.getQuantity()))).reduce(BigDecimal::add).get();

        cart1TotalAmount = cart1TotalAmount.add(bigDecimal1);
        ...
        BigDecimal bigDecimal2 = cartItemList1.stream()
                .map(cartItem -> cartItem.getProduct().getPrice().multiply(BigDecimal.valueOf(cartItem.getQuantity()))).reduce(BigDecimal::add).get();

        cart2TotalAmount = cart2TotalAmount.add(bigDecimal2);
